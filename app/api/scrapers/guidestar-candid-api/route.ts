...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'guidestar-candid-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'guidestar-candid-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for guidestar-candid-api:`, error)
    return NextResponse.json(
      { 
