...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'aviation-safety-network')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'aviation-safety-network',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for aviation-safety-network:`, error)
    return NextResponse.json(
      { 
