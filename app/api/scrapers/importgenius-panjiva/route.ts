...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'importgenius-panjiva')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'importgenius-panjiva',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for importgenius-panjiva:`, error)
    return NextResponse.json(
      { 
