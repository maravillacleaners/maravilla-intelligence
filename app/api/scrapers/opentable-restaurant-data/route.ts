...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'opentable-restaurant-data')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'opentable-restaurant-data',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for opentable-restaurant-data:`, error)
    return NextResponse.json(
      { 
