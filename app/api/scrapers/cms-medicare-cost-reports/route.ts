...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'cms-medicare-cost-reports')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'cms-medicare-cost-reports',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for cms-medicare-cost-reports:`, error)
    return NextResponse.json(
      { 
