...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'fmcsa-insurance-mcs-90')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'fmcsa-insurance-mcs-90',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for fmcsa-insurance-mcs-90:`, error)
    return NextResponse.json(
      { 
