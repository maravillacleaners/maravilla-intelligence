...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'forest-service-permits')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'forest-service-permits',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for forest-service-permits:`, error)
    return NextResponse.json(
      { 
