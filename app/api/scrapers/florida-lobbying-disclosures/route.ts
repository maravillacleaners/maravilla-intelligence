...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'florida-lobbying-disclosures')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'florida-lobbying-disclosures',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for florida-lobbying-disclosures:`, error)
    return NextResponse.json(
      { 
