...
/**
 * NEW YORK DEPARTMENT OF STATE SCRAPER
 * Recopila: nombre legal, DBA, fecha de formación, status, número de registro
 * Officer names, dirección, teléfono, email
 *
 * API público sin auth requerida - dos.ny.gov
 */
async function scrapeNYDOS(company_name: string, county?: string): Promise<NYDosResult[]> {
  console.log(`[NY_DOS] Buscando: ${company_name}${county ? ` en ${county}` : ''}`)

  try {
    const encodedName = encodeURIComponent(company_name)
    const url = `https://www.dos.ny.gov/cgi-bin/corp_public_search.cgi?company_name=${encodedName}&search_type=begins`
    ...
    if (!res.ok) {
      console.log(`[NY_DOS] HTTP ${res.status} para ${company_name}`)
      return []
    }

    const html = await res.text()
    const $ = cheerio.load(html)
    const results: NYDosResult[] = []
    ...
    $('table tbody tr').each((idx, el) => {
      try {
        ...
        console.error(`[NY_DOS] Parse error en fila ${idx}:`, e)
      }
    })

    console.log(`[NY_DOS] Encontrados ${results.length} resultados para ${company_name}`)
    return results
  } catch (error) {
    console.error(`[NY_DOS] Error para ${company_name}:`, error)
    return []
  }
}

/**
 * Enriquece resultado NY DOS con búsqueda detallada
 */
async function enrichNYDosResult(legal_name: string): Promise<Partial<NYDosResult>> {
  try {
    const searchUrl = `https://www.dos.ny.gov/cgi-bin/corp_public_search.cgi?company_name=${encodeURIComponent(legal_name)}&search_type=exact`
    ...
    const officers: NYDosResult['officers'] = []
    ...
    console.error(`[NY_DOS] Enrich error para ${legal_name}:`, e)
    return {}
  }
}
 ...
 */
async function saveToAirtable(results: NYDosResult[]) {
    ...
    try {
      const payload = {
        fields: {
          ...
          'data_source': 'NY_DOS',
          'investigator_notes': `New York Department of State record extracted ${new Date().toISOString()}`,
        },
      }
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Búsqueda inicial
    const results = await scrapeNYDOS(company_name, county)

    // Enriquecer cada resultado con detalles adicionales
    for (const result of results) {
      const enriched = await enrichNYDosResult(result.legal_name)
    ...
    return NextResponse.json({
      ...
      source: 'NY_DOS (New York Department of State)',
      timestamp: new Date().toISOString(),
    })
  } catch (error) {
    console.error('[NY_DOS API] Error:', error)
    return NextResponse.json(
...
export async function GET(req: NextRequest) {
  ...
  const results = await scrapeNYDOS(company_name, county || undefined)
  const savedIds = await saveToAirtable(results)
