...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'faa-aircraft-registry')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'faa-aircraft-registry',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for faa-aircraft-registry:`, error)
    return NextResponse.json(
      { 
