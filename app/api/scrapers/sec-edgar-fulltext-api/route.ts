...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'sec-edgar-fulltext-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'sec-edgar-fulltext-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for sec-edgar-fulltext-api:`, error)
    return NextResponse.json(
      { 
