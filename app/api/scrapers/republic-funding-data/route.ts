...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'republic-funding-data')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'republic-funding-data',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for republic-funding-data:`, error)
    return NextResponse.json(
      { 
