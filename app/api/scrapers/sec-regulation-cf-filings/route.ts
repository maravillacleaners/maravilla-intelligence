...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'sec-regulation-cf-filings')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'sec-regulation-cf-filings',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for sec-regulation-cf-filings:`, error)
    return NextResponse.json(
      { 
