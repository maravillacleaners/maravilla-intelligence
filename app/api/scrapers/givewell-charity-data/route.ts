...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'givewell-charity-data')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'givewell-charity-data',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for givewell-charity-data:`, error)
    return NextResponse.json(
      { 
