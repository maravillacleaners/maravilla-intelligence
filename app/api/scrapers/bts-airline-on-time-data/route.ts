...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'bts-airline-on-time-data')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'bts-airline-on-time-data',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for bts-airline-on-time-data:`, error)
    return NextResponse.json(
      { 
