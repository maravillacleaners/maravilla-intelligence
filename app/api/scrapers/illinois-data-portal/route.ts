...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'illinois-data-portal')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'illinois-data-portal',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for illinois-data-portal:`, error)
    return NextResponse.json(
      { 
