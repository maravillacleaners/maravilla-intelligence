...
/**
 * TEXAS SECRETARY OF STATE SCRAPER
 * Recopila: nombre legal, DBA, fecha de formación, status, número de registro
 * Officer names, dirección, teléfono, email
 *
 * API público sin auth requerida - sos.texas.gov
 */
async function scrapeTXSOS(company_name: string, county?: string): Promise<TXSosResult[]> {
  console.log(`[TX_SOS] Buscando: ${company_name}${county ? ` en ${county}` : ''}`)

  try {
    const encodedName = encodeURIComponent(company_name)
    const url = `https://www.sos.texas.gov/cgi-bin/corp_search_txcorp.pl?company_name=${encodedName}&search_type=begins&searchstate=TX`
    ...
    if (!res.ok) {
      console.log(`[TX_SOS] HTTP ${res.status} para ${company_name}`)
      return []
    }

    const html = await res.text()
    const $ = cheerio.load(html)
    const results: TXSosResult[] = []
    ...
    $('table tbody tr').each((idx, el) => {
      try {
        ...
        console.error(`[TX_SOS] Parse error en fila ${idx}:`, e)
      }
    })

    console.log(`[TX_SOS] Encontrados ${results.length} resultados para ${company_name}`)
    return results
  } catch (error) {
    console.error(`[TX_SOS] Error para ${company_name}:`, error)
    return []
  }
}

/**
 * Enriquece resultado TX SOS con búsqueda detallada
 */
async function enrichTXSosResult(legal_name: string): Promise<Partial<TXSosResult>> {
  try {
    const searchUrl = `https://www.sos.texas.gov/cgi-bin/corp_search_txcorp.pl?company_name=${encodeURIComponent(legal_name)}&search_type=exact&searchstate=TX`
    ...
    const officers: TXSosResult['officers'] = []
    ...
    console.error(`[TX_SOS] Enrich error para ${legal_name}:`, e)
    return {}
  }
}
 ...
 */
async function saveToAirtable(results: TXSosResult[]) {
    ...
    try {
      const payload = {
        fields: {
          ...
          'data_source': 'TX_SOS',
          'investigator_notes': `Texas Secretary of State record extracted ${new Date().toISOString()}`,
        },
      }
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Búsqueda inicial
    const results = await scrapeTXSOS(company_name, county)

    // Enriquecer cada resultado con detalles adicionales
    for (const result of results) {
      const enriched = await enrichTXSosResult(result.legal_name)
    ...
    return NextResponse.json({
      ...
      source: 'TX_SOS (Texas Secretary of State)',
      timestamp: new Date().toISOString(),
    })
  } catch (error) {
    console.error('[TX_SOS API] Error:', error)
    return NextResponse.json(
...
export async function GET(req: NextRequest) {
  ...
  const results = await scrapeTXSOS(company_name, county || undefined)
  const savedIds = await saveToAirtable(results)
