...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'aia-architect-finder')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'aia-architect-finder',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for aia-architect-finder:`, error)
    return NextResponse.json(
      { 
