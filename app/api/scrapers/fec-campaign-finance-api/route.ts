...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'fec-campaign-finance-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'fec-campaign-finance-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for fec-campaign-finance-api:`, error)
    return NextResponse.json(
      { 
