...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'str-smith-travel-research')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'str-smith-travel-research',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for str-smith-travel-research:`, error)
    return NextResponse.json(
      { 
