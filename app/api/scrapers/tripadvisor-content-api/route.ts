...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'tripadvisor-content-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'tripadvisor-content-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for tripadvisor-content-api:`, error)
    return NextResponse.json(
      { 
