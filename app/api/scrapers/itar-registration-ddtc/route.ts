...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'itar-registration-ddtc')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'itar-registration-ddtc',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for itar-registration-ddtc:`, error)
    return NextResponse.json(
      { 
