...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'treasury-ofac-sanctions')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'treasury-ofac-sanctions',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for treasury-ofac-sanctions:`, error)
    return NextResponse.json(
      { 
