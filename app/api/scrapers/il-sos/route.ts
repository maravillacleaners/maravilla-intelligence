...
/**
 * ILLINOIS SECRETARY OF STATE SCRAPER
 * Recopila: nombre legal, DBA, fecha de formación, status, número de registro
 * Officer names, dirección, teléfono, email
 *
 * API público sin auth requerida - cyberdriveillinois.com
 */
async function scrapeILSOS(company_name: string, county?: string): Promise<ILSosResult[]> {
  console.log(`[IL_SOS] Buscando: ${company_name}${county ? ` en ${county}` : ''}`)

  try {
    const encodedName = encodeURIComponent(company_name)
    const url = `https://www.cyberdriveillinois.com/cgi-bin/corp_search.pl?company_name=${encodedName}&search_type=begins`
    ...
    if (!res.ok) {
      console.log(`[IL_SOS] HTTP ${res.status} para ${company_name}`)
      return []
    }

    const html = await res.text()
    const $ = cheerio.load(html)
    const results: ILSosResult[] = []
    ...
    $('table tbody tr').each((idx, el) => {
      try {
        ...
        console.error(`[IL_SOS] Parse error en fila ${idx}:`, e)
      }
    })

    console.log(`[IL_SOS] Encontrados ${results.length} resultados para ${company_name}`)
    return results
  } catch (error) {
    console.error(`[IL_SOS] Error para ${company_name}:`, error)
    return []
  }
}

/**
 * Enriquece resultado IL SOS con búsqueda detallada
 */
async function enrichILSosResult(legal_name: string): Promise<Partial<ILSosResult>> {
  try {
    const searchUrl = `https://www.cyberdriveillinois.com/cgi-bin/corp_search.pl?company_name=${encodeURIComponent(legal_name)}&search_type=exact`
    ...
    const officers: ILSosResult['officers'] = []
    ...
    console.error(`[IL_SOS] Enrich error para ${legal_name}:`, e)
    return {}
  }
}
 ...
 */
async function saveToAirtable(results: ILSosResult[]) {
    ...
    try {
      const payload = {
        fields: {
          ...
          'data_source': 'IL_SOS',
          'investigator_notes': `Illinois Secretary of State record extracted ${new Date().toISOString()}`,
        },
      }
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Búsqueda inicial
    const results = await scrapeILSOS(company_name, county)

    // Enriquecer cada resultado con detalles adicionales
    for (const result of results) {
      const enriched = await enrichILSosResult(result.legal_name)
    ...
    return NextResponse.json({
      ...
      source: 'IL_SOS (Illinois Secretary of State)',
      timestamp: new Date().toISOString(),
    })
  } catch (error) {
    console.error('[IL_SOS API] Error:', error)
    return NextResponse.json(
...
export async function GET(req: NextRequest) {
  ...
  const results = await scrapeILSOS(company_name, county || undefined)
  const savedIds = await saveToAirtable(results)
