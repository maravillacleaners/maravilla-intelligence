...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'cbp-vessel-manifests')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'cbp-vessel-manifests',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for cbp-vessel-manifests:`, error)
    return NextResponse.json(
      { 
