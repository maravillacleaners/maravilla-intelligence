...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'faa-pilot-certification')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'faa-pilot-certification',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for faa-pilot-certification:`, error)
    return NextResponse.json(
      { 
