...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'health-inspection-records')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'health-inspection-records',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for health-inspection-records:`, error)
    return NextResponse.json(
      { 
