...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'usda-organic-integrity')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'usda-organic-integrity',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for usda-organic-integrity:`, error)
    return NextResponse.json(
      { 
