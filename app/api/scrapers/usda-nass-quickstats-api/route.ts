...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'usda-nass-quickstats-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'usda-nass-quickstats-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for usda-nass-quickstats-api:`, error)
    return NextResponse.json(
      { 
