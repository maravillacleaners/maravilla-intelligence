...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'propublica-congress-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'propublica-congress-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for propublica-congress-api:`, error)
    return NextResponse.json(
      { 
