...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'national-pipeline-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'national-pipeline-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for national-pipeline-api:`, error)
    return NextResponse.json(
      { 
