...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'amazon-seller-api-sp-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'amazon-seller-api-sp-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for amazon-seller-api-sp-api:`, error)
    return NextResponse.json(
      { 
