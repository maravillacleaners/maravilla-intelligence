...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'bea-foreign-direct-investment')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'bea-foreign-direct-investment',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for bea-foreign-direct-investment:`, error)
    return NextResponse.json(
      { 
