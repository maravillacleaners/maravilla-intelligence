...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'indiegogo-campaign-data')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'indiegogo-campaign-data',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for indiegogo-campaign-data:`, error)
    return NextResponse.json(
      { 
