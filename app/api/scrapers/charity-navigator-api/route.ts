...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'charity-navigator-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'charity-navigator-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for charity-navigator-api:`, error)
    return NextResponse.json(
      { 
