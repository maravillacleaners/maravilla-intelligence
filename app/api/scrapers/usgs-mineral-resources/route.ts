...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'usgs-mineral-resources')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'usgs-mineral-resources',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for usgs-mineral-resources:`, error)
    return NextResponse.json(
      { 
