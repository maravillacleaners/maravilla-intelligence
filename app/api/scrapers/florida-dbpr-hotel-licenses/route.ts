...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'florida-dbpr-hotel-licenses')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'florida-dbpr-hotel-licenses',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for florida-dbpr-hotel-licenses:`, error)
    return NextResponse.json(
      { 
