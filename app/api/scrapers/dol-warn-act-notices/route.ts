...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'dol-warn-act-notices')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'dol-warn-act-notices',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for dol-warn-act-notices:`, error)
    return NextResponse.json(
      { 
