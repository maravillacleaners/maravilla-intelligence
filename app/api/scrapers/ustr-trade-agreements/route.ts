...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'ustr-trade-agreements')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'ustr-trade-agreements',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for ustr-trade-agreements:`, error)
    return NextResponse.json(
      { 
