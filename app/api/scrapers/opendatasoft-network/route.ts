...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'opendatasoft-network')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'opendatasoft-network',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for opendatasoft-network:`, error)
    return NextResponse.json(
      { 
