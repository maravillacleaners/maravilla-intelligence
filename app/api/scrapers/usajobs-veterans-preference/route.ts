...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'usajobs-veterans-preference')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'usajobs-veterans-preference',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for usajobs-veterans-preference:`, error)
    return NextResponse.json(
      { 
