...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'usda-food-desert-locator')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'usda-food-desert-locator',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for usda-food-desert-locator:`, error)
    return NextResponse.json(
      { 
