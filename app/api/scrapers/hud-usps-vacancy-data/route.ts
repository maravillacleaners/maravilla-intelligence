...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'hud-usps-vacancy-data')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'hud-usps-vacancy-data',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for hud-usps-vacancy-data:`, error)
    return NextResponse.json(
      { 
