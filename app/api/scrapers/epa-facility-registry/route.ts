...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'epa-facility-registry')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'epa-facility-registry',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for epa-facility-registry:`, error)
    return NextResponse.json(
      { 
