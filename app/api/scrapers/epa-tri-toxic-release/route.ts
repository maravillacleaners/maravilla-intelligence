...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'epa-tri-toxic-release')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'epa-tri-toxic-release',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for epa-tri-toxic-release:`, error)
    return NextResponse.json(
      { 
