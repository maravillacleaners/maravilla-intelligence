...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'pacer-federal-court-records')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'pacer-federal-court-records',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for pacer-federal-court-records:`, error)
    return NextResponse.json(
      { 
