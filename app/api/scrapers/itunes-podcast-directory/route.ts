...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'itunes-podcast-directory')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'itunes-podcast-directory',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for itunes-podcast-directory:`, error)
    return NextResponse.json(
      { 
