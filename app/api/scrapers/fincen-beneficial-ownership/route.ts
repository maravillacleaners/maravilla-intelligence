...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'fincen-beneficial-ownership')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'fincen-beneficial-ownership',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for fincen-beneficial-ownership:`, error)
    return NextResponse.json(
      { 
