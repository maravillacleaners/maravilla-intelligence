...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'wcri-workers-comp-data')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'wcri-workers-comp-data',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for wcri-workers-comp-data:`, error)
    return NextResponse.json(
      { 
