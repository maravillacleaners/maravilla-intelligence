...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'ferc-electric-utilities')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'ferc-electric-utilities',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for ferc-electric-utilities:`, error)
    return NextResponse.json(
      { 
