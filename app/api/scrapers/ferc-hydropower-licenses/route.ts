...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'ferc-hydropower-licenses')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'ferc-hydropower-licenses',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for ferc-hydropower-licenses:`, error)
    return NextResponse.json(
      { 
