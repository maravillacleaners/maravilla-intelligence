...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'nih-reporter-research')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'nih-reporter-research',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for nih-reporter-research:`, error)
    return NextResponse.json(
      { 
