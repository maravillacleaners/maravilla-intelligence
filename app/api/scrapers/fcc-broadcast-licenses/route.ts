...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'fcc-broadcast-licenses')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'fcc-broadcast-licenses',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for fcc-broadcast-licenses:`, error)
    return NextResponse.json(
      { 
