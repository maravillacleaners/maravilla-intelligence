...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'finra-brokercheck-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'finra-brokercheck-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for finra-brokercheck-api:`, error)
    return NextResponse.json(
      { 
