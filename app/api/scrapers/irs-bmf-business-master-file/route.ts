...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'irs-bmf-business-master-file')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'irs-bmf-business-master-file',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for irs-bmf-business-master-file:`, error)
    return NextResponse.json(
      { 
