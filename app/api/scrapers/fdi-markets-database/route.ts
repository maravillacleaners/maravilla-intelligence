...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'fdi-markets-database')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'fdi-markets-database',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for fdi-markets-database:`, error)
    return NextResponse.json(
      { 
