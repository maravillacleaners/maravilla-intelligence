...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'water-utility-data-echo')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'water-utility-data-echo',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for water-utility-data-echo:`, error)
    return NextResponse.json(
      { 
