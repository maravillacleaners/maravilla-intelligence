...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'texas-sos-direct-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'texas-sos-direct-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for texas-sos-direct-api:`, error)
    return NextResponse.json(
      { 
