...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'usgs-national-map-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'usgs-national-map-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for usgs-national-map-api:`, error)
    return NextResponse.json(
      { 
