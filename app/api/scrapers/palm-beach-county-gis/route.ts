...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'palm-beach-county-gis')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'palm-beach-county-gis',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for palm-beach-county-gis:`, error)
    return NextResponse.json(
      { 
