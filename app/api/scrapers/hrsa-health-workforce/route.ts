...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'hrsa-health-workforce')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'hrsa-health-workforce',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for hrsa-health-workforce:`, error)
    return NextResponse.json(
      { 
