...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'bts-transportation-stats')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'bts-transportation-stats',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for bts-transportation-stats:`, error)
    return NextResponse.json(
      { 
