...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'cfpb-complaint-database')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'cfpb-complaint-database',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for cfpb-complaint-database:`, error)
    return NextResponse.json(
      { 
