...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'dod-transition-assistance')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'dod-transition-assistance',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for dod-transition-assistance:`, error)
    return NextResponse.json(
      { 
