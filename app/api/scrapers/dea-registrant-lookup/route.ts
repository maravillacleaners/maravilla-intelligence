...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'dea-registrant-lookup')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'dea-registrant-lookup',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for dea-registrant-lookup:`, error)
    return NextResponse.json(
      { 
