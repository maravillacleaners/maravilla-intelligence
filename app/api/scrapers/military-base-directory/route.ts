...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'military-base-directory')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'military-base-directory',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for military-base-directory:`, error)
    return NextResponse.json(
      { 
