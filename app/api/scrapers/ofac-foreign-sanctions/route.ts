...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'ofac-foreign-sanctions')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'ofac-foreign-sanctions',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for ofac-foreign-sanctions:`, error)
    return NextResponse.json(
      { 
