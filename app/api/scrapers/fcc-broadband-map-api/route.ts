...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'fcc-broadband-map-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'fcc-broadband-map-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for fcc-broadband-map-api:`, error)
    return NextResponse.json(
      { 
