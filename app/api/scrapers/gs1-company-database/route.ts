...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'gs1-company-database')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'gs1-company-database',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for gs1-company-database:`, error)
    return NextResponse.json(
      { 
