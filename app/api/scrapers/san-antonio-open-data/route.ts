...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'san-antonio-open-data')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'san-antonio-open-data',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for san-antonio-open-data:`, error)
    return NextResponse.json(
      { 
