...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'epa-rcra-hazardous-waste')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'epa-rcra-hazardous-waste',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for epa-rcra-hazardous-waste:`, error)
    return NextResponse.json(
      { 
