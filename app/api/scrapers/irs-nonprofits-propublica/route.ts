...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'irs-nonprofits-propublica')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'irs-nonprofits-propublica',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for irs-nonprofits-propublica:`, error)
    return NextResponse.json(
      { 
