...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'google-maps-places-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'google-maps-places-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for google-maps-places-api:`, error)
    return NextResponse.json(
      { 
