...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'phmsa-pipeline-operators')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'phmsa-pipeline-operators',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for phmsa-pipeline-operators:`, error)
    return NextResponse.json(
      { 
