...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'epa-pesticide-registrations')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'epa-pesticide-registrations',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for epa-pesticide-registrations:`, error)
    return NextResponse.json(
      { 
