...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'va-veteran-owned-business')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'va-veteran-owned-business',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for va-veteran-owned-business:`, error)
    return NextResponse.json(
      { 
