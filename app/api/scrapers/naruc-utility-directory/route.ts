...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'naruc-utility-directory')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'naruc-utility-directory',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for naruc-utility-directory:`, error)
    return NextResponse.json(
      { 
