...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'ncua-credit-union-data')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'ncua-credit-union-data',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for ncua-credit-union-data:`, error)
    return NextResponse.json(
      { 
