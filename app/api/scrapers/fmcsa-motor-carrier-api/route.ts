...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'fmcsa-motor-carrier-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'fmcsa-motor-carrier-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for fmcsa-motor-carrier-api:`, error)
    return NextResponse.json(
      { 
