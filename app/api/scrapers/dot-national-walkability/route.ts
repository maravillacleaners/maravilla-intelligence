...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'dot-national-walkability')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'dot-national-walkability',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for dot-national-walkability:`, error)
    return NextResponse.json(
      { 
