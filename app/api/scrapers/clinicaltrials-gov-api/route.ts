...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'clinicaltrials-gov-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'clinicaltrials-gov-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for clinicaltrials-gov-api:`, error)
    return NextResponse.json(
      { 
