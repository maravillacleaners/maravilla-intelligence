...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'cms-hospital-compare')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'cms-hospital-compare',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for cms-hospital-compare:`, error)
    return NextResponse.json(
      { 
