...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'federal-reserve-fred-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'federal-reserve-fred-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for federal-reserve-fred-api:`, error)
    return NextResponse.json(
      { 
