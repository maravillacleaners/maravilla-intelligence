...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'socrata-open-data-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'socrata-open-data-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for socrata-open-data-api:`, error)
    return NextResponse.json(
      { 
