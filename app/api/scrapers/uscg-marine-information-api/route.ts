...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'uscg-marine-information-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'uscg-marine-information-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for uscg-marine-information-api:`, error)
    return NextResponse.json(
      { 
