...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'bbb-accredited-business-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'bbb-accredited-business-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for bbb-accredited-business-api:`, error)
    return NextResponse.json(
      { 
