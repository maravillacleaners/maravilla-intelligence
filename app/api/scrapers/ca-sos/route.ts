...
/**
 * CALIFORNIA SECRETARY OF STATE SCRAPER
 * Recopila: nombre legal, DBA, fecha de formación, status, número de registro
 * Officer names, dirección, teléfono, email
 *
 * API público sin auth requerida - businesssearch.sos.ca.gov
 */
async function scrapeCASOS(company_name: string, county?: string): Promise<CASosResult[]> {
  console.log(`[CA_SOS] Buscando: ${company_name}${county ? ` en ${county}` : ''}`)

  try {
    const encodedName = encodeURIComponent(company_name)
    const url = `https://businesssearch.sos.ca.gov/cbs/searchresults?company=${encodedName}&state=CA`
    ...
    if (!res.ok) {
      console.log(`[CA_SOS] HTTP ${res.status} para ${company_name}`)
      return []
    }

    const html = await res.text()
    const $ = cheerio.load(html)
    const results: CASosResult[] = []
    ...
    $('table tbody tr').each((idx, el) => {
      try {
        ...
        console.error(`[CA_SOS] Parse error en fila ${idx}:`, e)
      }
    })

    console.log(`[CA_SOS] Encontrados ${results.length} resultados para ${company_name}`)
    return results
  } catch (error) {
    console.error(`[CA_SOS] Error para ${company_name}:`, error)
    return []
  }
}

/**
 * Enriquece resultado CA SOS con búsqueda detallada
 */
async function enrichCASosResult(legal_name: string): Promise<Partial<CASosResult>> {
  try {
    const searchUrl = `https://businesssearch.sos.ca.gov/cbs/searchresults?company=${encodeURIComponent(legal_name)}&state=CA`
    ...
    const officers: CASosResult['officers'] = []
    ...
    console.error(`[CA_SOS] Enrich error para ${legal_name}:`, e)
    return {}
  }
}
 ...
 */
async function saveToAirtable(results: CASosResult[]) {
    ...
    try {
      const payload = {
        fields: {
          ...
          'data_source': 'CA_SOS',
          'investigator_notes': `CA Secretary of State record extracted ${new Date().toISOString()}`,
        },
      }
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Búsqueda inicial
    const results = await scrapeCASOS(company_name, county)

    // Enriquecer cada resultado con detalles adicionales
    for (const result of results) {
      const enriched = await enrichCASosResult(result.legal_name)
    ...
    return NextResponse.json({
      ...
      source: 'CA_SOS (California Secretary of State)',
      timestamp: new Date().toISOString(),
    })
  } catch (error) {
    console.error('[CA_SOS API] Error:', error)
    return NextResponse.json(
...
export async function GET(req: NextRequest) {
  ...
  const results = await scrapeCASOS(company_name, county || undefined)
  const savedIds = await saveToAirtable(results)
