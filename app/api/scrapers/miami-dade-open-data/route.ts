...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'miami-dade-open-data')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'miami-dade-open-data',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for miami-dade-open-data:`, error)
    return NextResponse.json(
      { 
