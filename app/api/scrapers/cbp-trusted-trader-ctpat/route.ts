...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'cbp-trusted-trader-ctpat')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'cbp-trusted-trader-ctpat',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for cbp-trusted-trader-ctpat:`, error)
    return NextResponse.json(
      { 
