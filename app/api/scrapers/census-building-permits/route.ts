...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'census-building-permits')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'census-building-permits',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for census-building-permits:`, error)
    return NextResponse.json(
      { 
