...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'dtic-technical-reports')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'dtic-technical-reports',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for dtic-technical-reports:`, error)
    return NextResponse.json(
      { 
