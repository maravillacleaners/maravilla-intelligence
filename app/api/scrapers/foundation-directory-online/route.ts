...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'foundation-directory-online')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'foundation-directory-online',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for foundation-directory-online:`, error)
    return NextResponse.json(
      { 
