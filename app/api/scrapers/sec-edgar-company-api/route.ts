...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'sec-edgar-company-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'sec-edgar-company-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for sec-edgar-company-api:`, error)
    return NextResponse.json(
      { 
