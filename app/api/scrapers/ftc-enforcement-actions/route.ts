...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'ftc-enforcement-actions')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'ftc-enforcement-actions',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for ftc-enforcement-actions:`, error)
    return NextResponse.json(
      { 
