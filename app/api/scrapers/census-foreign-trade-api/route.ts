...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'census-foreign-trade-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'census-foreign-trade-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for census-foreign-trade-api:`, error)
    return NextResponse.json(
      { 
