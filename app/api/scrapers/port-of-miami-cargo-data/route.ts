...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'port-of-miami-cargo-data')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'port-of-miami-cargo-data',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for port-of-miami-cargo-data:`, error)
    return NextResponse.json(
      { 
