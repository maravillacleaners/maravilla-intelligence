...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'sec-investment-advisers')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'sec-investment-advisers',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for sec-investment-advisers:`, error)
    return NextResponse.json(
      { 
