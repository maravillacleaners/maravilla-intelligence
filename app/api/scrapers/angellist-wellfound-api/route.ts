...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'angellist-wellfound-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'angellist-wellfound-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for angellist-wellfound-api:`, error)
    return NextResponse.json(
      { 
