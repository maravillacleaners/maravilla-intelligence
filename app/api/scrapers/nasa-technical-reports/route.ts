...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'nasa-technical-reports')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'nasa-technical-reports',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for nasa-technical-reports:`, error)
    return NextResponse.json(
      { 
