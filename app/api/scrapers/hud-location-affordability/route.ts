...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'hud-location-affordability')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'hud-location-affordability',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for hud-location-affordability:`, error)
    return NextResponse.json(
      { 
