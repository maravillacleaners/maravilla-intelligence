...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'osha-enforcement-api')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'osha-enforcement-api',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for osha-enforcement-api:`, error)
    return NextResponse.json(
      { 
