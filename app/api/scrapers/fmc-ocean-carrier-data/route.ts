...
export async function POST(req: NextRequest) {
  try {
    const config = configs.find((c: any) => c.source_id === 'fmc-ocean-carrier-data')
    if (!config) {
    ...
    return NextResponse.json({
      success: true,
      source_id: 'fmc-ocean-carrier-data',
      timestamp: new Date().toISOString(),
      data: result
    })
  } catch (error) {
    console.error(`Scraper error for fmc-ocean-carrier-data:`, error)
    return NextResponse.json(
      { 
